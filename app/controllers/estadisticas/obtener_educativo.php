...
if (isset($_GET['grado'])) {
    ...
    try {
        ...
        $query = "SELECT COUNT(*) AS total FROM estudiantes as est INNER JOIN abandonos AS aba ON est.id_estudiante = aba.estudiante_id INNER JOIN grados AS gra ON gra.id_grado = est.grado_id WHERE grado_id = :grado AND aba.abandono = 1 AND aba.motivo = 'FACTORES EDUCATIVOS'";
